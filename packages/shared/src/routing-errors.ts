...
export function formatThreadReplyHelpMessage(): string {
  return 'Thread replies must be natural language, /new, or /end.';
}

export function formatThreadTaskCommandRejectedMessage(): string {
  return 'Cannot use /task in a thread. Reply with natural language, /new, or /end.\nUse /task only as a new root message.';
}

export function formatThreadOnlyCommandMessage(command: '/new' | '/end'): string {
