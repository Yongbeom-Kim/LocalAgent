...
describe('registerSubmitCommand', () => {
  ...
  it('wires explicit executor and apiUrl from CLI options into submitTask', async () => {
    ...
    await program.parseAsync(['submit', '--payload', 'test prompt', '--executor', 'ttadk', '--api-url', 'http://example.com:3000'], {
      from: 'user',
    });
  ...
  it('uses DEFAULT_API_URL when apiUrl option and API_URL env are unset', async () => {
    ...
    await program.parseAsync(['submit', '--payload', 'test prompt', '--executor', 'claude_code'], {
      from: 'user',
    });
