...
export class TTCodexExecutor implements TaskExecutor {
  ...
  private spawnTTCodex(
    ...
    return new Promise((resolve) => {
      const child = spawn('ttadk', args, { cwd: env.workDir, shell: false });
      ...
      child.on('close', (code) => {
        const sanitizedStdout = this.sanitizeOutput(Buffer.concat(stdoutChunks).toString());
        const sanitizedStderr = this.sanitizeOutput(Buffer.concat(stderrChunks).toString());
