...
describe('loadLarkListenerConfig', () => {
  ...
  it('throws when required env vars are missing', () => {
    expect(() => loadLarkListenerConfig({ API_URL: 'http://localhost:3000', LARK_APP_SECRET: 'secret456' })).toThrow(
      'LARK_APP_ID is required',
    );
    expect(() => loadLarkListenerConfig({ API_URL: 'http://localhost:3000', LARK_APP_ID: 'app123' })).toThrow(
      'LARK_APP_SECRET is required',
    );
    expect(() => loadLarkListenerConfig({ LARK_APP_ID: 'app123', LARK_APP_SECRET: 'secret456' })).toThrow(
      'API_URL is required',
    );
  });
});
