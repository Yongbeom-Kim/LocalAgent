...
> Refinement note: for the current Lark contract, `/new` and `/new <executor> <model>` are thread-only commands. Root-message behavior and natural-language thread continuation are refined in `docs/development/design/2026-04-02-lark-thread-natural-language-continuation-design.md`.
